...
class HashTable:
    ...
    def delete(self, key):
        ...
        self.check_size() 
        bucket_index = calculate_hash(key) % self.bucket_size
        item = self.buckets[bucket_index] #head
   ...
   #rehash     # wrong one but dont know why...　
    ...
    def expand(self):
        ...
        for old_item in old_buckets :
            current = old_item 
            while current:
                bucket_index = calculate_hash(current.key) % self.bucket_size
                new_item = Item(current.key, current.value, self.buckets[bucket_index])
                self.buckets[bucket_index] = new_item
